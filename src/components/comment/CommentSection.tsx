...
export default function CommentSection({ postId }: CommentSectionProps) {
  ...
  const Spinner = () => (
    <div className="w-8 h-8 border border-tertiary border-t-primary rounded-full animate-spin" />
  );
  ...
  return (
    <div className="space-y-6 sm:space-y-10">
      <div className="bg-card border border-border p-6 sm:p-8 shadow-md rounded-lg transition-all-smooth hover:shadow-lg">
        {isAuthenticated ? (
          <CommentForm onSubmit={(content) => handleSubmitComment(content)} />
        ) : (
          <div className="text-center py-6">
            <p className="text-[11px] sm:text-xs font-sans text-secondary mb-6">
              Login with GitHub TO LEAVE A COMMENT
            </p>
            <button
              onClick={login}
              className="btn-interactive px-4 sm:px-6 py-2 sm:py-3 border border-border text-[11px] sm:text-xs font-sans text-text bg-card hover:bg-primary hover:text-white transition-all-smooth inline-flex items-center gap-2 sm:gap-3 shadow-md rounded-lg hover:shadow-lg hover:-translate-y-0.5 active:translate-y-0"
            >
      ...
      <div className="space-y-4 sm:space-y-6">
        {comments.length === 0 ? (
          <div className="border-t border-border pt-8 sm:pt-12 text-center">
            <p className="text-[11px] sm:text-xs font-sans text-muted">No comments yet.</p>
          </div>
        ) : (
